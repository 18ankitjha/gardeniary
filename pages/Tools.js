    ...
    <div>
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          ...
          {Object.keys(products).length === 0 && <p className="text-center">Sorry all Tools are out of stock</p>}
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in Tools) {
      if (!Tools[item.title].color.includes(item.color) && item.availableQty > 0) {
        Tools[item.title].color.push(item.color)

      }
      if (!Tools[item.title].size.includes(item.size) && item.availableQty > 0) {
        Tools[item.title].size.push(item.size)
      }
    } else {
      Tools[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availableQty > 0) {
        Tools[item.title].color = [item.color]
        Tools[item.title].size = [item.size]
      }
    }
  }
  // console.log(Tools);
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(Tools)) }, // will be passed to the page component as props
  }
}
