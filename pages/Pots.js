      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && <p className="text-2xl text-center">Sorry all Pots are out of stock</p>}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "Pots" })
  console.log(products);
  let Pots = {}
  for (let item of products) {
    if (item.title in Pots) {
      if (!Pots[item.title].color.includes(item.color) && item.availableQty > 0) {
        Pots[item.title].color.push(item.color)

      }
      if (!Pots[item.title].size.includes(item.size) && item.availableQty > 0) {
        Pots[item.title].size.push(item.size)
      }
    } else {
      Pots[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availableQty > 0) {
        Pots[item.title].color = [item.color]
        Pots[item.title].size = [item.size]
      }
    }
  }
  // console.log(Pots);
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(Pots)) }, // will be passed to the page component as props
  }
}
export default Pots